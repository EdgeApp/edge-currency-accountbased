...
import { cleanTxLogs, getFetchCors, getOtherParams, matchJson } from '../common/utils'
...
> {
  ...
  async queryBalance(): Promise<void> {
    try {
      ...
      if (networkUpdate.balances != null && networkUpdate.balances[address] != null) {
        const balance = networkUpdate.balances[address]
        this.updateBalance(this.currencyInfo.currencyCode, balance)
      }
      
      if (networkUpdate.blockHeight != null && networkUpdate.blockHeight > this.walletLocalData.blockHeight) {
  ...
  async makeSpend(edgeSpendInfoIn: EdgeSpendInfo): Promise<EdgeTransaction> {
    ...
    let totalInput = '0'
    const targetAmount = add(nativeAmount, edgeSpendInfo.networkFeeOption || this.networkInfo.defaultFee)
  ...
  async broadcastTx(
    ...
    try {
      const result = await this.kaspaNetwork.broadcastTx(edgeTransaction.signedTx)
