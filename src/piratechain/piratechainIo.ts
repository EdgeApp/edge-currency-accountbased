...
const asTransactionInfo = asObject<TransactionInfo>({
  txid: asString,
    ...
    async makeSynchronizer(config) {
      ...
      const out: PiratechainSynchronizer = bridgifyObject({
        ...
        getTransactions: async () => {
          ...
          // starts as `[]` and the engine only asks after the poller has
          // filled it, so there is no RPC to fall back to; a malformed row is
          // dropped on its own rather than rejecting the whole history:
          ...
          if (transactions.length < raw.length) {
            console.warn(
              `piratechain: dropped ${
                raw.length - transactions.length
