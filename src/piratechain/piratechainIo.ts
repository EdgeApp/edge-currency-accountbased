  ...
  return bridgifyObject<PiratechainIo>({
    ...
    async makeSynchronizer(config) {
      ...
      const out: PiratechainSynchronizer = bridgifyObject({
        ...
        stop: async () => {
          try {
            ...
            // when the signing key must leave memory, whether or not the
            // poller closed cleanly:
            await walletSdk
      ...
      try {
        ...
        // poller or lock the signing key that was unlocked above. Without
        // this a retry would leave a second synchronizer running against the
        // same registry wallet, and the key would stay in memory past the
        // account lock that would otherwise clear it through `stop`:
        await realSynchronizer.close().catch(() => undefined)
        await walletSdk.lockWalletSigning(walletId).catch(() => undefined)
        throw error
      }
      return out
    }
