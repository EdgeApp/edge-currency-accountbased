...
export function mnemonicMatchesKeysSeed(
  ...
  const decoded = mnemonicToSeed(normalized, true).slice(
    0,
...
export function validateMnemonic(mnemonic: string): void {
  ...
  if (!verifySeedPhrase(normalized)) {
