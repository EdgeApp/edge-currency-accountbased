...
// const TIMESTAMP_BEFORE_ZEC_LAUNCH = 1554076800000 // 2019-04-01, ZEC launched on 2019-04-18
// const NATIVE_UNIT_MULTIPLIER = '100000000'
// const TRANSACTION_QUERY_TIME_WINDOW = 1000 * 60 * 60 * 24 * 2 * 28 // two months
// const NETWORK_FEE_NATIVE_AMOUNT = '37500' // fixed amount for ZEC
...
export class ZcashEngine extends CurrencyEngine {
  ...
  //       `/api/v1/node-info`
  //     )
  //     const valid = validateObject(jsonObj, ZcashApiNodeInfo)
  //     if (valid) {
  ...
  //     )
  //     const valid = validateObject(jsonObj, ZcashApiAccountBalance)
  //     if (valid) {
  ...
  //       this.transactionList.ZEC.length === 0
  //     ) {
  //       this.updateBalance('ZEC', '0')
  //     }
  //     this.log.error(`Error checking ZEC address balance`)
  //   }
  // }

  // processZcashApiTransaction(tx: ZcashApiTransaction, currencyCode: string) {
  //   let netNativeAmount: string // Amount received into wallet
  //   const ourReceiveAddresses: string[] = []
  //   const nativeNetworkFee: string = bns.mul(tx.txFee, NATIVE_UNIT_MULTIPLIER) // always denominated in ZEC
  ...
  //           transactionsResults,
  //           ZcashApiGetTransactions
  //         )
  //         if (valid) {
  //           for (const transaction of transactionsResults.tx) {
  //             // shuold we process extra transaction for native ZEC fees?
  //             this.processZcashApiTransaction(transaction, currencyCode)
  //           }
  ...
  //   let startTime: number = TIMESTAMP_BEFORE_ZEC_LAUNCH
  //   const promiseArray = []
  ...
  // async multicastServers(func: ZECFunction, ...params: any): Promise<any> {
  //   let out = { result: '', server: 'no server' }
  //   let funcs
  //   switch (func) {
  //     case 'zec_broadcastTx': {
  //       const promises = []
  //       const broadcastServers = this.currencyInfo.defaultSettings.otherSettings
  //         .zcashApiServers
  //       for (const zecServer of broadcastServers) {
  //         const endpoint = `${zecServer}/api/v1/broadcast?sync=true`
  //         promises.push(
  ...
  //     case 'zec_blockNumber':
  //     case 'zec_getBalance':
  //     case 'zec_getTransactions':
  //       funcs = this.currencyInfo.defaultSettings.otherSettings.zcashApiServers.map(
  //         server => async () => {
  ...
  async makeSpend(edgeSpendInfoIn: EdgeSpendInfo) {
    ...
    //   const zecParams: ZcashTxOtherParams = {
    ...
    // const ErrorInsufficientFundsMoreZEC = new Error(
    //   'Insufficient ZEC for transaction fee'
    // )
    // ErrorInsufficientFundsMoreZEC.name = 'ErrorInsufficientFundsMoreZEC'

    // let nativeAmount = edgeSpendInfo.spendTargets[0].nativeAmount
    // const balanceZEC = this.walletLocalData.totalBalances[
    //   this.currencyInfo.currencyCode
    // ]

    // let totalTxAmount = '0'
    // totalTxAmount = bns.add(nativeAmount, nativeNetworkFee)
    // if (bns.gt(totalTxAmount, balanceZEC)) {
  ...
  async signTx(edgeTransaction: EdgeTransaction): Promise<EdgeTransaction> {
    ...
    // const zecClient = new ZECApiClient(
    //   currencyInfo.defaultSettings.otherSettings.zcashApiServers[0]
    // )
    // zecClient.chooseNetwork('mainnet')
    // const privKey = this.walletInfo.keys.zcashKey
    // await zecClient.setPrivateKey(privKey)
    // await zecClient.initChain()
    ...
    // zecClient._broadcastDelegate = x => {
    //   return x
    // }
    // // WILL NOT ACTUALLY TRANSFER! That will be done in this.broadcastTx
    // const signedTx = await zecClient.transfer(
    //   otherParams.from[0],
    //   otherParams.to[0],
    //   nativeAmount,
    //   currencyCode,
  ...
  async broadcastTx(
    ...
    // const zecSignedTransaction = otherParams.serializedTx
    // const response = await this.multicastServers(
    //   'zec_broadcastTx',
    //   zecSignedTransaction
    // )
