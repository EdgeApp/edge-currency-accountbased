...
sequenceDiagram
  ...
  Bridge->>SDK: enableWalletSigningProtection on first sight, then lockWalletSigning
...
[`src/piratechain/PiratechainEngine.ts`](https://github.com/EdgeApp/edge-currency-accountbased/blob/6973f77d3b200fe9f9cb9dc0804c77203e2595d3/src/piratechain/PiratechainEngine.ts)
```ts
...
The SDK synchronizer emits its first `statusChanged` (`STOPPED` to `SYNCING`) synchronously inside `start()`, so a listener attached afterwards never sees it and the engine would sit at `STOPPED`, which blocks every spend. The bridge therefore exposes `start` on the handle instead of starting inside `makeSynchronizer`: `syncNetwork` creates the synchronizer, subscribes in `initSubscriptions`, and only then calls `start()`, so no event can fire into the void. An earlier revision started inside `makeSynchronizer` and had `initSubscriptions` read the status once afterwards, adopting it only while the engine still said `STOPPED`; that worked, but it detected the race rather than removing it, and the review on 2026-09-11 asked for the ordering fix.

The handle is the only way to stop the native poller, so a `start()` that throws closes the synchronizer before rethrowing: without that, a retry stacks a second poller on the same registry wallet.
...
So `makeSpend` gates on it. `checkSpendable` keeps the existing `SYNCED` check, then reads the status and throws `PendingFundsError` naming which wait the wallet is in. Both refusals are the same error type: a wallet still scanning has no spend anchor either, so the pre-sync branch throws `PendingFundsError` with the same syncing text rather than a plain `Error`, which is what keeps the amount modal and the send scene's error card on one message instead of sending an unsynced wallet to the generic network-failure card. Refusing at `makeSpend` rather than at broadcast is what makes this a disabled send instead of a failed one: the send scene keeps its confirm slider disabled while it holds no transaction, so the user never confirms a spend the SDK is going to reject. The GUI turns that error into localized retry copy ([section 6](#6-detailed-design-edge-react-gui-and-the-sdk-dependency)) in place of the generic "check your network connection" card.
...
Nothing in the package runs that assembly on install. `0.2.1` invoked it from a `postinstall`, which `edge-react-gui` could never fire because `.npmrc` sets `ignore-scripts=true`; `0.3.0` dropped the hook. What runs it is the package's podspec: top-level Ruby in a `:path:` podspec is evaluated on every `pod install`, and `0.3.4`'s calls the assembly script with `--force` and raises if the framework directory is still missing afterwards. `npm run prepare.ios` runs `pod install` right after `prepare.sh`, so the framework is assembled when CocoaPods needs it, on the machine that needs it. Earlier revisions also called the script from `scripts/prepare.sh`; that line was redundant with the podspec and weaker than it (no `--force`, no check that the output exists), and the review on 2026-09-11 had it removed.
...
So `SendScene2` translates that one error, in the same `makeSpend` catch block that already translates `InsufficientFundsError`, into an `I18nError` that carries the plugin's own message when it set one, and `send_funds_not_spendable_error_message` (the funds are not spendable yet, wait for the wallet to finish syncing and try again) only when the error arrived without text. The plugin picks its message from the SDK's `reasonCode`, so a wallet rebuilding its witness data says so rather than telling the user to wait for sync, which the review on 2026-09-11 pointed out an unconditional substitution had flattened. Monero throws the same error for unconfirmed funds with edge-core-js's default text, "Not enough confirmed funds", which the scene now shows as-is; the localized line remains the fallback.
...
SDK 0.3.2 adds a multi-server mode: instead of one endpoint per wallet, the sync engine takes a pool and spreads block fetches across the servers in it, moving off one that stalls. It is off by default upstream. The plugin calls it through the wrapper's typed `setLightdEndpointPool(walletId, url, failoverEndpoints)`, which sends the [RPC](#rpc) `set_lightd_endpoint_pool` with the wallet id, the primary URL and the alternates. Earlier revisions sent the raw RPC with an extra `automatic_failover: true`; the service's handler (`pirate-wallet-service/src/api.rs`, `set_lightd_endpoint_pool`) takes no such argument and sets `automatic_failover = !failover_endpoints.is_empty()` itself, so the flag was always ignored and a non-empty pool is what turns failover on. The single-endpoint call and the chain-tip probe go through the typed `setLightdEndpoint` and `testLightdEndpoint` for the same reason, which also removes the hand-written snake_case cleaner the raw `test_node` result needed.
...
|---|---|---|---|---|
| Debug (`0.3.0`, 2026-08-17) | iphonesimulator | 187.3MB | 97.6MB | fails to link |
| Debug (`0.3.4`, 2026-09-07) | iphonesimulator | 190.0MB | 100.2MB | fails to link |
...
Re-measured on `0.3.4` with every library autolinked and no trim: the same fixup error from the same two symbols, with the displacement at 177,426,652 bytes and `text_env` at `0x0ADA62E0`, 2.2MB further out than on `0.3.0`. `__TEXT` grew from 187.3MB to 190.0MB and `__text` from 97.6MB to 100.2MB, so the branch is moving away from the limit rather than toward it, and `react-native-monero`'s changelog through 0.5.0 carries no change to the `text_env` section.

Static-library contributions at Debug are roughly: pirate-wallet 369MB, zcash 106MB, monero 51MB, zano 29MB. This is not specific to this branch (the same libraries linked on 2026-08-04), and it reproduces on `0.3.0` (simulator slice about 1MB larger than `0.2.1`) and on `0.3.4`. Local Debug testing gets past it by excluding `react-native-zcash`, `react-native-monero` and `react-native-zano` from iOS autolinking, which is a test-only measure and is never committed.
...
25. Endpoint pool diagnostic (VERIFIED, iOS sim, 2026-09-14). On 2026-09-02 the app's persisted log files carried no plugin lines, so the probe went unobserved. This round captured the core's log events through a local `onLog` hook (test scaffolding, not committed) and the line arrived one second after the synchronizer started, through the typed `setLightdEndpointPool`: `automaticFailover: true`, `configuredEndpoint` `lightwalletd2.cryptoforge.cc`, `activeEndpoint` `lightwalletd1.cryptoforge.cc`, with the primary marked unhealthy (`502 Bad Gateway` behind nginx) and the alternate healthy at 1,679 ms with tip 4,134,292. That is the first observation of automatic failover actually moving a wallet, the case item 19 could not construct: the primary was down on its own that evening, and the wallet synced to `SYNCED` over the alternate.
26. Broadcast during a primary outage (OBSERVED, iOS sim, 2026-09-14). With sync running over the alternate, two slides of a 0.044 ARRR self-send both failed inside the SDK with `Broadcast failed: h2 protocol error: error reading a body from connection` from the same nginx front, and a direct gRPC probe from the host confirmed the state: `lightwalletd2.cryptoforge.cc` answered 502 while `lightwalletd1.cryptoforge.cc` and `lightd1.pirate.black` answered gRPC. So `broadcast_tx` goes to the configured primary and does not follow the sync engine's failover; the send scene showed the generic network-error card, which is the right diagnosis here. Raised with the Pirate team below.
27. Funded send on the review-fix build (VERIFIED, iOS sim, real broadcast, 2026-09-14). With the primary and alternate swapped in the installed engine asset (a local edit to the throwaway build, restored afterwards) so the healthy node was primary, the same 0.044 ARRR self-send from `My Pirate` reached "Transaction Success" on the first slide, txid `0fc26e01781020bb994164f31526a4d21c9a8434e6bdafcd6f1ec4bacb9a8abe`. This build carries the send-scoped unlock (the key is unlocked inside `send` and locked in its `finally`), `start()` called by the engine after `initSubscriptions`, and the typed endpoint wrappers, so the send is evidence for all three.
...
- **Link ceiling re-measured, 2026-09-07:** one untrimmed Debug simulator build of the branch on `0.3.4` fails to link with the same `arm64_b26` fixup error as on 2026-08-17, 2.2MB further out of range ([section 6](#the-arm64-debug-link-ceiling)). The local three-library trim stays the way to build Debug for the simulator; Release is unaffected.

### Phase 11: the first human review

- **Asked (2026-09-11, nine threads on the plugin PR and three on the GUI PR):** compile against the package's own types instead of a hand-written ambient declaration; declare `create-hmac`; use the typed endpoint wrappers; gate `getMaxSpendable` like `makeSpend`; record a transaction's seen height only after it is processed; stop unlocking the signing key at start; expose `start` on the bridge instead of the status sentinel; revert the dead `rpcNode.defaultHost`; drop the redundant `prepare.sh` assembly line; state the dependency footprint; keep the plugin's spendability message in the send scene.
- **Shipped:** all of it, folded into the commits that own each surface, with one divergence: protection is still enabled at first sight rather than lazily inside `send`, so a wallet that never sends still has its spending keys wrapped at rest ([wallet signing sessions](#wallet-signing-sessions)); the key itself is now unlocked only for the span of a send. `SpendabilityStatus` types its anchor heights as `number`, but a finalizing wallet reports `null` (observed on the simulator, 2026-09-05), so the plugin keeps its own cleaner for that result rather than trusting the declaration.
- **Found on the way:** the `automatic_failover: true` the raw pool call carried since phase 7 never did anything; the service derives the flag from the alternates list ([the failover pool](#the-lightwalletd-failover-pool)). The Debug simulator build also needs the three excluded native modules' io factories stubbed on the GUI side, since `makeMoneroIo` and `makeZanoIo` throw at construction without their native module and take `EdgeCoreManager` down with them; that is test scaffolding, never committed.

### Phase 12: the re-review

- **Asked (2026-09-15, two threads on the plugin PR):** throw `PendingFundsError` rather than a plain `Error` for the pre-sync refusal, since the send scene's `ErrorCard` renders a plain one as a network failure worth reporting; clean the poller's transaction snapshot per entry instead of rejecting the whole array and falling back to an uncleaned `listTransactions`.
...
- Funded send after the first human review, with the key unlocked only for the span of the send, txid [`0fc26e01781020bb994164f31526a4d21c9a8434e6bdafcd6f1ec4bacb9a8abe`](https://explorer.pirate.black/tx/0fc26e01781020bb994164f31526a4d21c9a8434e6bdafcd6f1ec4bacb9a8abe) ([section 7](#7-testing) item 27), and the first observed automatic failover, during a real primary outage (item 25).
