...
import {
//   EthGasStationSchema,
  DexBinanceOrgGetAccount,
  //   EtherscanGetAccountNonce,
  DexBinanceOrgNodeInfo
...
export class BinanceEngine extends CurrencyEngine {
  ...
  async fetchGetDexBinanceOrg (server: string, cmd: string) {
  ...
  async checkBlockchainInnerLoop () {
    try {
      const jsonObj = await this.multicastServers('bnb_blockNumber')
      const valid = validateObject(jsonObj, DexBinanceOrgNodeInfo)
      if (valid) {
  ...
  async checkAccountFetch (address: string) {
    ...
    try {
      jsonObj = await this.multicastServers('bnb_getBalance', address)
      valid = validateObject(jsonObj, DexBinanceOrgGetAccount)
      if (valid) {
        const balances = jsonObj.balances
        const balanceInfo = balances.find(balance => balance.symbol === 'BNB')
        this.updateBalance('BNB', balanceInfo.free)
      }
    } catch (e) {
      this.log(`Error checking token balance: BNB`)
    }
  }
  ...
  async multicastServers (func: EthFunction, ...params: any): Promise<any> {
    ...
    switch (func) {
      ...
      //   const promises = []
      //   promises.push(
      //     broadcastWrapper(this.broadcastDexBinanceOrg(params[0]), 'dex-binance-org')
      //   )
      ...
      case 'bnb_blockNumber':
        funcs = this.currencyInfo.defaultSettings.otherSettings.binanceApiServers.map(
          server => async () => {
            ...
            //   )
            // }
            const result = await this.fetchGetDexBinanceOrg(
              server,
              '/node-info'
            )
            const valid = validateObject(result, DexBinanceOrgNodeInfo)
            if (!valid) {
              const msg = `Invalid return value bnb_blockNumber in ${server}`
              this.log(msg)
              throw new Error(msg)
            }
      ...
      case 'bnb_getTransactionCount':
        ...
        funcs = this.currencyInfo.defaultSettings.otherSettings.binanceApiServers.map(
          server => async () => {
            ...
            const result = await this.fetchGetDexBinanceOrg(server, url)
  ...
  //   }`
  //   const jsonObj = await this.fetchGetDexBinanceOrg(
