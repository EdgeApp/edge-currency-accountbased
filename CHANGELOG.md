...
- added: (ARRR) A lightwalletd failover pool, on by default now that a second reachable node exists. Sync spreads across the configured nodes and moves off one that stalls. The SDK validates a pool before accepting it and refuses one that mixes transports or names a host it does not recognize; in that case the wallet falls back to its single node rather than failing the sync. The info server can replace the list without an app release.
...
- changed: (ARRR) Require `react-native-pirate-wallet` 0.3.4. The 0.3 line sets the Ironwood activation height and ships the Supernova sync engine; 0.3.2 adds sync recovery, fixes a sync stall and a network timeout, and splits the iOS simulator binaries into one package per architecture; 0.3.4 keeps a queued witness repair visible in the spendability status until the node will accept the wallet's anchor, scopes broadcast to the wallet that built the transaction, and adds keychain-backed registry storage and wallet signing sessions.
- changed: (ARRR) Keep the registry credential in the iOS Keychain or Android Keystore. The native module mints it and it never crosses into JavaScript, replacing the random passphrase the plugin kept as plaintext on its local storage. Existing dev-device registries are abandoned; wallets restore from their seeds.
- changed: (ARRR) Lock each wallet's signing keys whenever its engine is not running. The SDK wraps the seed and spending keys under a credential the plugin derives from account-encrypted key material it only holds while the account is unlocked, so a registry at rest can sync but cannot sign.
- changed: (ARRR) Log which lightwalletd the SDK selected from the pool, with each node's health, tip and latency, once per wallet start.
- changed: (ARRR) Scan and broadcast over TLS, against `lightwalletd2.cryptoforge.cc`. The node this replaces served plain gRPC, so a network observer could learn which block ranges a device fetched and when it broadcast, and an active attacker could serve a forked view. It was also much slower under the current SDK: a cold scan of a wallet 1.06 million blocks behind took over half an hour there and 95 seconds against the new node, measured on the same device minutes apart.
- fixed: (ARRR) Refuse a send while the wallet is still finalizing, instead of failing it. A Pirate Chain wallet reports fully synced before its spend anchor is usable, and a send in that window used to build, get confirmed by the user, and then fail inside the SDK with a message the app showed as a network error. The plugin now asks the SDK whether the wallet can spend and declines to build the transaction until it can, which keeps the confirm slider disabled and the reason accurate. A broadcast that still fails because the wallet is not ready reports the same way, so the advice is to wait and retry rather than to check the network.
- fixed: (ARRR) Point each Pirate Chain wallet at the plugin's own lightwalletd node. The SDK ships a default node and never reads the plugin's configuration, so wallets scanned against that default instead; when it stopped serving blocks the wallet sat at "Sync in Progress, 0% Complete" indefinitely with no error, since the chain tip still resolved.
