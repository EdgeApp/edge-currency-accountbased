...
describe('mnemonicMatchesKeysSeed', () => {
  ...
  it('refuses a passphrase-protected phrase', () => {
    // `mnemonicToSeed` does not decrypt, and answers a protected phrase with
    // a wrong seed rather than an error, so this would otherwise report a
    // meaningless comparison as a plain false.
    assert.throws(
      () =>
        mnemonicMatchesKeysSeed(withTimestampWord(mnemonic, 'among'), zeroSeed),
      /requires a passphrase/
    )
  })
...
describe('validateMnemonic', () => {
  ...
  it('rejects a 26-word phrase whose checksum does not match', () => {
    // The realistic transcription error: one word mistyped into another word
    // that is also in the dictionary. The native import path this replaced
    // rejected these, so accepting them would import a different account
    // under the user's phrase.
    const words = mnemonic.split(' ')
    words[25] = 'shoulder'
    assert.throws(() => validateMnemonic(words.join(' ')), /checksum/)
  })
  ...
  it('treats the last unflagged timestamp word as unprotected', () => {
    // The checksum is salted with the timestamp word, so swapping it
    // invalidates the checksum and this reaches the later check. Failing
    // there rather than on the passphrase check is the point: index 799 is
    // still unflagged.
    assert.throws(
  ...
  it('rejects a phrase whose checksum word does not match', () => {
    // The native path this replaced rejected these:
    // `account_base::restore_from_seed_phrase` returns false on a checksum
    // mismatch, and `get_seed_phrase_info` then reports an empty address,
    // which the old `importPrivateKey` threw on. One word mistyped onto
    // another dictionary word would otherwise import a different account.
    assert.throws(
      () =>
        validateMnemonic(
