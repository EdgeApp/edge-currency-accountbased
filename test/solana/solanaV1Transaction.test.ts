...
interface StubOptions {
  ...
  /**
   * Makes this node answer every `getTransaction` the same way: a null result
   * (a node that has not caught up), an outage, or an unsupported version.
   */
  transactionAnswer?: 'null' | 'unhealthy' | 'unsupported'
}
...
const makeStubConnection = (options: StubOptions = {}): Connection => {
  ...
  const answer = (request: JsonRpcRequest): unknown => {
    ...
    if (transactionAnswer === 'null') return reply({ result: null })
    if (transactionAnswer === 'unhealthy') {
      return reply({ error: nodeUnhealthyError })
    }
    if (transactionAnswer === 'unsupported') {
...
describe('Solana transaction chunk fetch', function () {
  ...
  it('reports a signature no node holds as unavailable', async function () {
    const engine = makeFakeEngine(makeStubConnection(), FEE_PAYER)
    const { transactions, unavailable } = await fetchChunk(engine, [
      MISSING_SIGNATURE,
      V1_SIGNATURE
    ...
    // A null can come from a node that has not caught up yet, so it is not
    // taken as the final word on a signature the address just listed.
    assert.equal(transactions.length, 2)
    assert.isNull(transactions[0])
    assert.equal(transactions[1].transaction.signatures[0], V1_SIGNATURE)
    assert.deepEqual([...unavailable], [MISSING_SIGNATURE])
  })

  it('asks the next node when the first answers null', async function () {
    const engine = makeFakeEngine(makeStubConnection(), FEE_PAYER)
    engine.tools.archiveConnections = [
      makeStubConnection({ transactionAnswer: 'null' }),
      makeStubConnection()
    ]
    const { transactions, unavailable } = await fetchChunk(engine, [
      V1_SIGNATURE
    ...
    assert.equal(transactions[0].transaction.signatures[0], V1_SIGNATURE)
    assert.equal(unavailable.size, 0)
  })

  it('keeps an unsupported version answer when another node is down', async function () {
    const engine = makeFakeEngine(makeStubConnection(), FEE_PAYER)
    // The outage is the last node to fail, which is the error a plain race
    // would reject with.
    engine.tools.archiveConnections = [
      makeStubConnection({ transactionAnswer: 'unsupported' }),
      makeStubConnection({ transactionAnswer: 'unhealthy' })
    ]
    const { transactions, unavailable } = await fetchChunk(engine, [
      V1_SIGNATURE
    ])
...
describe('Solana history watermark', function () {
  ...
  it('moves past an unsupported transaction version that is the newest row', async function () {
    const engine = makeHistoryEngine(
      makeStubConnection({ history: [UNSERVABLE_SIGNATURE, V1_SIGNATURE] })
    )
    await syncHistory(engine)

    // Without this the next poll would ask for the unreadable row again, and
    // again on every poll after it.
    assert.equal(engine.otherData.newestTxid[''], UNSERVABLE_SIGNATURE)
    assert.equal(engine.captured.length, 1)
  })

  it('holds still behind a row no node holds yet', async function () {
    const engine = makeHistoryEngine(
      makeStubConnection({ history: [V1_SIGNATURE, MISSING_SIGNATURE] })
    )
    await syncHistory(engine)
  ...
  it('drops a row the nodes say has no block time', async function () {
    ...
    // row is dropped rather than recorded against a placeholder timestamp, and
