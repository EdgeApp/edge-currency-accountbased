...
describe('RetryWithBackoff', () => {
  let retryHelper: RetryWithBackoff

  beforeEach(() => {
    retryHelper = new RetryWithBackoff()
  })
  ...
  describe('execute', () => {
    ...
    it('should not retry if error is not retriable', async () => {
      let attempts = 0
      ...
      try {
        ...
        expect(error.message).to.equal('Not retriable')
    ...
    it('should cancel retry when AbortSignal is triggered', async () => {
      const abortController = new AbortController()
      let attempts = 0

      // Start retry operation
      const retryPromise = retryHelper.execute(
        async () => {
          attempts++
      ...
      try {
        ...
        expect(attempts).to.be.at.least(2).and.at.most(3) // Should have attempted 2-3 times before cancellation
      }
    })

    it('should support infinite retries when maxRetries is not specified', async () => {
      let attempts = 0
      const retryHelperNoLimit = new RetryWithBackoff()
      const result = await retryHelperNoLimit.execute(
        async () => {
          attempts++
          if (attempts < 10) {
        ...
        },
        {
          ...
          maxDelay: 20,
          backoffFactor: 2,
          maxRetries: undefined // Explicitly set to undefined for infinite retries
        }
      )
