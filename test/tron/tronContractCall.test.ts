...
describe('confirmed tron contract calls', function () {
  it('reads the call value from a confirmed contract call', function () {
    const contract = asTriggerSmartContract({
      type: 'TriggerSmartContract',
      parameter: {
        value: {
          data: 'cef95229',
          owner_address: '416c4d3cb629599f55e634bcbe08c9ffc560373d77',
          contract_address: '414ab38f7ae7eadad03981b2a7d7883760aa63e564',
          call_value: 14709000
        }
      }
    })
    assert.equal(contract.parameter.value.call_value, 14709000)
  })
  ...
  it('defaults a missing call value to zero', function () {
    ...
    assert.equal(contract.parameter.value.call_value, 0)
  })

  it('counts the TRX a successful call sent plus its fee', function () {
    assert.equal(contractCallNativeAmount(14709000, 1416400, true), '-16125400')
  })

  it('counts only the fee when the call failed', function () {
    assert.equal(contractCallNativeAmount(14709000, 1416400, false), '-1416400')
  })

  it('counts a token call as its fee alone', function () {
